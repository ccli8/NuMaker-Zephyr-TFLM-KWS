...
/* On zephyr, use semaphore to signal there are new samples available
 * instead of busy-wait
 */
#if defined(__ZEPHYR__)
K_SEM_DEFINE(dmic_sem_update, 0, 1);
#endif
...
#if (NVT_DCACHE_ON == 1)
    /*
     * Due to DTCM being non-cacheable, allocate LPPDMA descriptor at
     * DTCM can spare cache coherency handling.
     */
#if defined(__ZEPHYR__)
    __attribute__((section(".dtcm_bss.dmic_lppdma")))
...
#if defined(__ZEPHYR__)
    /* New samples are coming */
    k_sem_give(&dmic_sem_update);
#endif
...
static void lppdma_isr(void *dev)
{
    ARG_UNUSED(dev);
    LPPDMA_IRQHandler();
}
...
)
{
    ...
    /* On zephyr, go in zephyr way */
#if defined(__ZEPHYR__)
    IRQ_CONNECT(LPPDMA_IRQn, 0, lppdma_isr, NULL, 0);
    irq_enable(LPPDMA_IRQn);
#else
...
#if defined(__ZEPHYR__)
// Sleep until there are new samples
int32_t DMICRecord_SleepUntilUpdate(uint32_t timeout_ms)
{
    return (k_sem_take(&dmic_sem_update, K_MSEC(timeout_ms)) == 0) ? 0 : -1;
}
#endif
